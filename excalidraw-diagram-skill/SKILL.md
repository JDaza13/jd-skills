...
**In this repo, use the Node renderer** — it runs on the Playwright already installed here and needs no setup:

```bash
node .claude/skills/excalidraw-diagram/references/render_excalidraw.mjs <path-to-file.excalidraw> [--scale 1]
```

The Python original is equivalent but requires `uv`, which is not installed here:
